    ...
    def __init__(self, obs: dict[str, NDArray[np.floating]], info: dict, config: dict):
        ...
        for gate_quad, gate_pos in zip(gates_quat, gates_positions):
            ...
            for obst_pos in obstacle_positions:
                ...
                if d <= d_obstacle:
                    # Add approach point
                    detour_start = self.detour_point(beginning_of_line, end_of_line, obst_pos, d_obstacle)
                    way_points.append(detour_start)
                    
                    # Add maximum clearance point
                    detour_mid = self.detour_point(beginning_of_line, end_of_line, obst_pos, 1.5*d_obstacle)
                    way_points.append(detour_mid)
                    
                    # Add return point
                    detour_end = self.detour_point(end_of_line, beginning_of_line, obst_pos, d_obstacle)
                    way_points.append(detour_end)
